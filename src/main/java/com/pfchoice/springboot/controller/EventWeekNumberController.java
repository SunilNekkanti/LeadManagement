...
public class EventWeekNumberController {
	...
	@Autowired
	EventWeekNumberService eventWeekNumberService; //Service which will do all data retrieval/manipulation work

	// -------------------Retrieve All EventWeekNumbers---------------------------------------------
	...
	// -------------------Retrieve Single EventWeekNumber------------------------------------------
	...
	public ResponseEntity<?> getEventWeekNumber(@PathVariable("id") int id) {
		...
		if (eventWeekNumber == null) {
			...
			return new ResponseEntity(new CustomErrorType("EventWeekNumber with id " + id 
					+ " not found"), HttpStatus.NOT_FOUND);
		}
		return new ResponseEntity<EventWeekNumber>(eventWeekNumber, HttpStatus.OK);
	}

	// -------------------Create a EventWeekNumber-------------------------------------------
	@Secured({  "ROLE_ADMIN" })
	@RequestMapping(value = "/eventWeekNumber/", method = RequestMethod.POST)
	public ResponseEntity<?> createEventWeekNumber(@RequestBody EventWeekNumber eventWeekNumber, UriComponentsBuilder ucBuilder) {
		...
		if (eventWeekNumberService.isEventWeekNumberExist(eventWeekNumber)) {
			...
			return new ResponseEntity(new CustomErrorType("Unable to create. A EventWeekNumber with name " + 
			eventWeekNumber.getId() + " already exist."),HttpStatus.CONFLICT);
		}
		...
		headers.setLocation(ucBuilder.path("/api/eventWeekNumber/{id}").buildAndExpand(eventWeekNumber.getId()).toUri());
	...
	public ResponseEntity<?> updateEventWeekNumber(@PathVariable("id") int id, @RequestBody EventWeekNumber eventWeekNumber) {
		...
		if (currentEventWeekNumber == null) {
			...
			return new ResponseEntity(new CustomErrorType("Unable to upate. EventWeekNumber with id " + id + " not found."),
	...
	public ResponseEntity<?> deleteEventWeekNumber(@PathVariable("id") int id) {
		...
		if (eventWeekNumber == null) {
			...
			return new ResponseEntity(new CustomErrorType("Unable to delete. EventWeekNumber with id " + id + " not found."),
					HttpStatus.NOT_FOUND);
		}

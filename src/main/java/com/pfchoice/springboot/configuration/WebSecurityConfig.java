		...
		http.csrf().disable()
		       .authorizeRequests()
		         .expressionHandler(new CustomWebSecurityExpressionHandler())
		          .antMatchers("/api/**").access("(hasAnyRole('ROLE_CARE_COORDINATOR','ROLE_EVENT_COORDINATOR')  and hasAnyIpAddress('108.190.27.18') ) or hasAnyRole('ROLE_AGENT','ROLE_MANAGER', 'ROLE_ADMIN') ").anyRequest()

	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.csrf().disable()
		       .authorizeRequests()
		         ...
		         .antMatchers("/api/**").access("(hasAnyRole('ROLE_CARE_COORDINATOR','ROLE_EVENT_COORDINATOR')  and hasAnyIpAddress('35.143.38.108') ) or hasAnyRole('ROLE_AGENT','ROLE_MANAGER', 'ROLE_ADMIN') ").anyRequest()
		         //     .antMatchers("/api/**").access("(hasAnyRole('ROLE_CARE_COORDINATOR')  and hasAnyIpAddress('192.168.1.131') ) or hasAnyRole('ROLE_AGENT','ROLE_MANAGER', 'ROLE_ADMIN','ROLE_EVENT_COORDINATOR') ").anyRequest()
